define('reports/profits', ['reports/base', 'jquery', 'common/util', 'handlebars'], function($base, $, $util, handlebars) {
    var hbs = handlebars.default;
    var proto = {
        init: function () {
            var ndForm = $('.J-reports-filter-form');
            ndForm.attr('action', '/api/reports/categoryinout');
            $base.init();
            $base.initPreview(function (pdata) {
                var data = pdata.data;
                var tplData = {
                    title: '利润表预览',
                    start: pdata.timeintervalStart.replace(/-/g, ''),
                    end: pdata.timeintervalEnd.replace(/-/g, ''),
                    now: $util.parseDate(new Date).replace(/-/g, '')
                };
                var tbody = [];
                tplData.tbody = createBody(data.items);
                tplData.thead = [{width: '25%', value: '总利润'}, { width: '50%', value: data.total}];
                var html = $util.tpl('#J-report-table-tpl', tplData);
                $('.J-report-setting').after(html);
            });
            function createBody(data) {
                var tbody = [];
                var totalAmount;
                for (var i in data) {
                    if (!data.hasOwnProperty(i)) continue;
                    totalAmount = data[i].total;
                    tbody.push({
                        data: [i, totalAmount],
                        children: getSubData(data[i].data)
                    });
                }
                return tbody;
            }
            function getSubData(data) {
                var result = [];
                if (!data) return result;
                var child;
                for (var i = 0; i < data.length; i++) {
                    child = [];
                    child.push(data[i].category);
                    child.push((data[i].totalStr));
                    result.push(child);
                }
                return result;
            }
        }
    };
    return proto;
});
